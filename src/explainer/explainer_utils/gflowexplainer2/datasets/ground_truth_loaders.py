...
def load_dataset_ground_truth(_dataset, shuffle=True, test_indices=None):
    ...
    if _dataset == "syn1" or _dataset == "syn2":
        graph, labels, adj = _load_node_dataset_ground_truth(_dataset)
        if test_indices is None:
            return (graph, labels), range(400, 700, 5), adj
        else:
            all = range(400, 700, 1)
            all_list = list(all)
            sample_number = int(len(all) * test_indices)
            test_indices =  random.sample(all_list, sample_number)
            filtered = [i for i in all if i in test_indices]
            filtered_2 = [i for i in all if i not in filtered]
            return (graph, labels), filtered,filtered_2
    if _dataset == "syn3":
