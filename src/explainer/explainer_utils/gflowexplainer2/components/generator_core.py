...
class GraphConv:
    ...
    def forward(self, x: sp.spmatrix):
        ...
        for _ in range(self.k):
            x = self.alpha * (self.normlized_adj_mat @ x) + (1 - self.alpha) * init_val  # Equation(2) in paper , (700,128)
        return x
...
class Agent(nn.Module):
    ...
    def __init__(self, hidden_size, with_attr=False, attr_size=-1, norm_type=None):
        super().__init__()
        self.hidden_size = hidden_size
        self.with_attr = with_attr
        self.attr_size = attr_size # 10
        if with_attr:
            self.attr_embedding = nn.Linear(attr_size, hidden_size)
        ...
        # self.Z = make_mlp([envs[0].num_cond_dim] + [self.n_hid // 2] * self.n_layers + [1])
    ...
    def forward(self, x_attrs, x_seeds, x_nodes, indptr):
        ...
        # print('x_seeds',x_seeds)
        h = self.seed_embedding(x_seeds.unsqueeze(1)) + self.node_embedding(x_nodes.unsqueeze(1)) # Todo: 可以考虑把这里的 x_seed给删除
        #
        # h =self.node_embedding(x_nodes.unsqueeze(1))
        ...
        for startpoint, endpoint, candidate_endpoint in indptr:
            if startpoint == endpoint:
                ...
                value = self.value_layer(global_z).squeeze() # 这里可能是公式(10)的stop criteria
                ...
                logits = torch.cat([node_logits + stopping_logits[0], stopping_logits[1:]], dim=0)  # 为什么要这么计算 # [*+1]
